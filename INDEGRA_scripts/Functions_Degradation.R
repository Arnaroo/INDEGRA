...
Test_Degradation<-function(process1,process2,p)
{
	...
	mergedFrag <- merge(x=Frag1, y=Frag2, by="transcript")
	...
	FilteredK=mergedFrag[is.element(mergedFrag$transcript,TranscriptK),]

	B= FilteredK %>% group_by(transcript)  %>%
...
PlotResults<-function(ResultObject,samplenames=c("sample 1","sample 2"))
{
